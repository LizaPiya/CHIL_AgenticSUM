...
def llama_judge_flan_t5_evaluation(csv_path="Flan_t5_summaries_full_dataset.csv", output_path="flan_t5_judge_results.csv"):
    """
    Hallucination evaluation of Flan-T5 summaries using Llama 3 8B as judge
    """
    ...
    # Load your Flan-T5 results
    df = pd.read_csv(csv_path)
    print(f"📄 Loaded {len(df)} Flan-T5 generated summaries")
    print(f"📋 Columns: {list(df.columns)}")
    
    # Store results
    results = []
    
    print("🔍 Evaluating Flan-T5 summaries for hallucinations...")
    
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Evaluating"):
        try:
            # Get your data columns - adapted for your CSV structure
            source = str(row['original_input'])  # The clinical note
            summary = str(row['generated_summary'])  # Flan-T5 generated summary
...
if __name__ == "__main__":
    ...
    print("Running Flan-T5 evaluation...")
    results = llama_judge_flan_t5_evaluation()
    print("Done!")