...
def evaluate_flan_t5_summaries(df, summary_column='generated_summary', reference_column='target'):
    """
    Evaluate Flan-T5-generated summaries with comprehensive metrics including standard deviations
    
    Args:
        df: DataFrame with MIMIC evaluation data (from Flan-T5 generation script)
        summary_column: Column name for generated summaries (default: 'generated_summary')
        reference_column: Column name for reference summaries (default: 'target')
    """
    
    print(f"📊 Evaluating {len(df)} Flan-T5-generated summaries...")
...
def analyze_token_lengths(df):
    ...
    if 'summary_token_count' in df.columns:
        ...
        print(f"  Within 140-160: {len(df[(token_counts >= 140) & (token_counts <= 160)])}/{len(df)} ({len(df[(token_counts >= 140) & (token_counts <= 160)])/len(df)*100:.1f}%)")
...
# Main evaluation function for MIMIC/Flan-T5 data
def run_flan_t5_evaluation(csv_path="Flan_t5_summaries_full_dataset.csv", output_path=None):
    """
    Run complete evaluation on Flan-T5-generated MIMIC summaries
    """
    print("🚀 Starting Flan-T5 MIMIC Evaluation")
    print("="*80)
    ...
    # Run evaluation
    df_evaluated = evaluate_flan_t5_summaries(df, reference_column='target_summary')
...
if __name__ == "__main__":
    # Run evaluation on your Flan-T5-generated MIMIC dataset
    df_results = run_flan_t5_evaluation("Flan_t5_summaries_full_dataset.csv")
    
    print("\n🎉 Flan-T5 evaluation completed!")
