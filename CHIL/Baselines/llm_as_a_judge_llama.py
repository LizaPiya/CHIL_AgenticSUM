...
def llama_judge_3_2_evaluation(csv_path="llama_3.2_3b_summaries_full_dataset.csv", output_path="llama_3.2_3b_judge_results.csv"):
    """
    Hallucination evaluation of Llama-3.2-3B summaries using Llama 3 8B as judge
    """
    ...
    # Load your Llama-3.2-3B results
    df = pd.read_csv(csv_path)
    print(f"📄 Loaded {len(df)} Llama-3.2-3B generated summaries")
    print(f"📋 Columns: {list(df.columns)}")
    
    # Store results
    results = []
    
    print("🔍 Evaluating Llama-3.2-3B summaries for hallucinations...")
    
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Evaluating"):
        try:
            # Get your data columns - adapted for your CSV structure
            source = str(row['original_input'])  # The clinical note
            summary = str(row['generated_summary'])  # Llama-3.2-3B generated summary
...
Coherent: X"""
    ...
    print("\n📊 LLAMA-3.2-3B HALLUCINATION EVALUATION RESULTS:")
    print("=" * 70)
...
def test_single_llama_sample(csv_path="llama_3.2_3b_summaries_full_dataset.csv", sample_idx=0):
    """
    Test evaluation on a single Llama-3.2-3B sample to debug
    """
    print("🧪 Testing single Llama-3.2-3B sample evaluation...")
    
    # Load data
    df = pd.read_csv(csv_path)
    sample = df.iloc[sample_idx]
    
    print(f"Testing sample {sample_idx} from {len(df)} total samples")
    print(f"Note ID: {sample['note_id']}")
    print(f"Summary token count: {sample['summary_token_count']}")
    print(f"Summary word count: {sample.get('summary_word_count', 'N/A')}")
    ...
    # Get data
    source = str(sample['original_input'])
    summary = str(sample['generated_summary'])
...
Coherent: X"""
    ...
    print(f"\n📊 EXTRACTED SCORES:")
    print(f"Hallucination: {halluc_match.group(1) if halluc_match else 'Not found'}")
    print(f"Factual: {factual_match.group(1) if factual_match else 'Not found'}")
    print(f"Complete: {complete_match.group(1) if complete_match else 'Not found'}")
    print(f"Coherent: {coherent_match.group(1) if coherent_match else 'Not found'}")

if __name__ == "__main__":
    # Test single sample first
    print("Testing single Llama-3.2-3B sample...")
    test_single_llama_sample()
    
    # Ask user if they want to proceed
    proceed = input("\nDoes the test look good? Run full evaluation? (y/n): ")
    
    if proceed.lower() == 'y':
        print("\n" + "="*60)
        print("Running full Llama-3.2-3B evaluation...")
        results = llama_judge_3_2_evaluation()
    else:
        print("Stopped. Fix any issues and try again.")