...
# ======================================================
# Load dataset - RERUN SKIPPED ONLY
# ======================================================
data_path = "../Dataset/sample_data_100.csv"
skipped_ids = pd.read_csv("/lustre/hl/users/4283/outputs/agenticsum/skipped_note_ids.csv")["note_id"].tolist()
df = pd.read_csv(data_path)
df = df[df["note_id"].isin(skipped_ids)].reset_index(drop=True)

print(f"Rerunning {len(df)} previously skipped notes\n")
...
os.makedirs(f"{output_dir}/checkpoints_rerun", exist_ok=True)

final_output_path = f"{output_dir}/agenticsum_results_rerun.csv"
...
# ======================================================
print("="*80)
print("Starting AgenticSum Pipeline - RERUN")
print("="*80 + "\n")
...
with torch.no_grad():
    for batch_start in range(0, len(df), BATCH_SIZE):
        ...
        for idx, row in batch_df.iterrows():
            try:
                print(f"[{idx+1}/{len(df)}] Processing {row['note_id']} ({len(row['input'])} chars)...", end=" ")
            ...
            # Aggressive cleanup after each note (important for long docs)
            torch.cuda.empty_cache()
            gc.collect()

        # Save checkpoint after each batch
        checkpoint_path = f"{output_dir}/checkpoints_rerun/checkpoint_batch_{batch_start//BATCH_SIZE + 1}.csv"
...
success_count = len([r for r in results if "ERROR" not in str(r.get("fixed_summary", ""))])
error_count = len([r for r in results if "ERROR" in str(r.get("fixed_summary", ""))])

print("\n" + "="*80)
print(f"✅ RERUN COMPLETE")
print(f"Total: {len(results)}/38")
print(f"Successful: {success_count}")
print(f"Errors (OOM): {error_count}")
